...
namespace NzbDrone.Core.Test.Datastore
{
    ...
    public class WhereBuilderFixture : CoreTest
    {
        ...
        public void where_throws_without_concrete_condition_if_requiresConcreteCondition()
        {
            ...
            Subject = new WhereBuilder(filter, true);
            Assert.Throws<InvalidOperationException>(() => Subject.ToString());
        }
        ...
        public void where_allows_abstract_condition_if_not_requiresConcreteCondition()
        {
            var movie = new Movie();
            Expression<Func<Movie, bool>> filter = (x) => x.Id == movie.Id;
            Subject = new WhereBuilder(filter, false);
            Subject.ToString().Should().Be($"(\"Movies\".\"Id\" = \"Movies\".\"Id\")");
        }
        ...
        {
            var test = "small";
            Subject = Where(x => x.CleanTitle.StartsWith(test));
        ...
        {
            var test = "small";
            Subject = Where(x => x.CleanTitle.EndsWith(test));
